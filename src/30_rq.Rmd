...
releases_bproj <- releases |>
    ...
    summarize(
        ...
        commits_in_cycle = 1 - mean(commits_in_advance) / mean(commits),
...
```{r}
wilcox.test(
    commits_in_cycle ~ cycle_type, data = releases_bproj,
    paired = TRUE)
cliff.delta(
    commits_in_cycle ~ cycle_type, data = releases_bproj)
...
releases_bproj |>
    group_by(cycle_type) |>
    summarize(
        quantile(commits_in_cycle, 0.5),
        ecdf(commits_in_cycle)(1))
...
releases_bproj |>
    ...
    summarize(
        ...
        min = q1 - 1.5 * iqr,
        max = q3 + 1.5 * iqr,
        1 - ecdf(commits_in_cycle)(1),
        ecdf(commits_in_cycle)(0),
    ) |> t()
