...
releases |>
    ungroup() |>
    select(cycle_days, commits, duration, start_delay, patches) |>
    cor(method = "spearman")
```
...
plot
ggsave("duration_dist.png", path = FIG_PATH,
...
releases_bproj |>
    group_by(cycle_type) %>%
    summarize(
        shapiro.test(commits_in_advance)$p.value,
        shapiro.test(commits_in_advance)$p.value)
```

```{r}
wilcox.test(
    commits_in_advance ~ cycle_type, data = releases_bproj,
    paired = TRUE)
cliff.delta(
    commits_in_advance ~ cycle_type, data = releases_bproj)
...
releases_bproj |>
    group_by(cycle_type) |>
    summarize(
        quantile(commits_in_advance, 0.5),
        ecdf(commits_in_advance)(1))

releases_bproj |>
    group_by(cycle_type) |>
    summarize(
        m = median(commits_in_advance),
        q1 = quantile(commits_in_advance, 0.25),
        q2 = quantile(commits_in_advance, 0.5),
        q3 = quantile(commits_in_advance, 0.75),
        iqr = q3 - q1,
        min = q1 - 1.5 * iqr,
        max = q3 + 1.5 * iqr,
        1 - ecdf(commits_in_advance)(1),
        ecdf(commits_in_advance)(0),
    ) |> t()
...
plot
ggsave("commits_in_advance.png", path = FIG_PATH,
    height = 80, unit = "mm", dpi = 300)
```