...
rapid_releases_parallel <- rapid_releases |> filter(duration_nrm > 1)

rapid_developed_releases_parallel <- rapid_releases_parallel |> filter(duration_abs < 6*7)

rapid_developed_releases_parallel |> nrow() / rapid_releases_parallel |> nrow()
...
## Rapid releases by delay

```{r}

releases |>
    filter(start_delay > 6*7 & duration < 6*7)


releases |>
    filter(trad_release == 'True')
